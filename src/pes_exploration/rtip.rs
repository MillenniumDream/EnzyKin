...
//! Of note, only the rotation is taken into account, excluding the inversion.
...
pub fn rti_dist(coord1: &Array2<f64>, coord2: &Array2<f64>) -> f64
{
    ...
    // When the eigenvalues are 0, they can also be -0, so taking absolute value is necessary before taking square root
    eigvals[0].abs().sqrt()
}
...
pub fn rti_dists(coord1: &Array2<f64>, coord2: &Array2<f64>) -> Vec<f64>
{
    ...
    // When the eigenvalues are 0, they can also be -0, so taking absolute value is necessary before taking square root
    vec![eigvals[0].abs().sqrt(), eigvals[1].abs().sqrt(), eigvals[2].abs().sqrt(), eigvals[3].abs().sqrt()]
}
...
//    assert_eq!(coord2.len_of(Axis(1)), 3);
    ...
    // When the eigenvalues are 0, they can also be -0, so taking absolute value is necessary before taking square root
    return (eigvals[0].abs().sqrt(), rti_vec)
}
...
//    assert_eq!(coord2.len_of(Axis(1)), 3);
    ...
    // When the eigenvalues are 0, they can also be -0, so taking absolute value is necessary before taking square root
    return (vec![eigvals[0].abs().sqrt(), eigvals[1].abs().sqrt(), eigvals[2].abs().sqrt(), eigvals[3].abs().sqrt()], vec![rti_vec0, rti_vec1, rti_vec2, rti_vec3])
}

/// Input the atomic coordinates of two systems (coord1, coord2),
/// output the minimum roto-translational invariant (RTI) distance between the two atomic coordinates
/// under corresponding transformation (rot, tran), which makes ( coord2 - (coord1 * rot + tran) ) minimum,
/// output the transformed (coord1 * rot + tran).
///
/// # Parameters
/// ```
/// coord1: the atomic coordinates of System 1 (natom * 3)
/// coord2: the atomic coordinates of System 2 (natom * 3)
/// rti_dist: the output minimum roto-translational invariant (RTI) distance between coord1 and coord2
/// coord_1: the output transformed (coord1 * rot + tran)
/// ```
pub fn rti_dist_coord(coord1: &Array2<f64>, coord2: &Array2<f64>) -> (f64, Array2<f64>)
{
//    assert_eq!(coord1.len_of(Axis(0)), coord2.len_of(Axis(0)));
//    assert_eq!(coord1.len_of(Axis(1)), 3);
//    assert_eq!(coord2.len_of(Axis(1)), 3);
    ...
    let coord_1: Array2<f64> = coord1.dot(&rot) + o2;

    // When the eigenvalues are 0, they can also be -0, so taking absolute value is necessary before taking square root
    return (eigvals[0].abs().sqrt(), coord_1)
}
...
/// ```
pub fn rti_dist_rot_tran(coord1: &Array2<f64>, coord2: &Array2<f64>) -> (f64, Array2<f64>, Array1<f64>)
{
//    assert_eq!(coord1.len_of(Axis(0)), coord2.len_of(Axis(0)));
//    assert_eq!(coord1.len_of(Axis(1)), 3);
//    assert_eq!(coord2.len_of(Axis(1)), 3);
        ...
        match (&self.visited_states[0].atom_add_pot, &s.atom_add_pot)
        {
            ...
            (None, None) =>
            {
                ...
                for i in 0..self.visited_states.len()
                {
                    if i % size == rank
                    {
                        pot_rtip_rank += rti_pot(&self.visited_states[i].coord, &s.coord, self.a[i], self.sigma[i]);
                    }
                }
            ...
            (None, Some(atom_add_pot_s)) =>
            {
                ...
                for i in 0..self.visited_states.len()
                {
                    if i % size == rank
                    {
                        pot_rtip_rank += rti_pot(&self.visited_states[i].coord, &s_fragment_coord, self.a[i], self.sigma[i]);
                    }
                }
            ...
            (Some(atom_add_pot_states), None) =>
            {
                ...
                for i in 0..self.visited_states.len()
                {
                    if i % size == rank
                    {
                        ...
                        pot_rtip_rank += rti_pot(&visited_states_coord, &s.coord, self.a[i], self.sigma[i]);
                    }
                }
            ...
            (Some(atom_add_pot_states), Some(atom_add_pot_s)) =>
            {
                ...
                for i in 0..self.visited_states.len()
                {
                    if i % size == rank
                    {
                        ...
                        pot_rtip_rank += rti_pot(&visited_states_coord, &s_fragment_coord, self.a[i], self.sigma[i]);
                    }
                }
        ...
        match (&self.visited_states[0].atom_add_pot, &s.atom_add_pot)
        {
            ...
            (None, None) =>
            {
                ...
                for i in 0..self.visited_states.len()
                {
                    if i % size == rank
                    {
                        (pot_rtip_state, force_rtip_state) = rti_pot_force(&self.visited_states[i].coord, &s.coord, self.a[i], self.sigma[i]);
            ...
            (None, Some(atom_add_pot_s)) =>
            {
                ...
                for i in 0..self.visited_states.len()
                {
                    if i % size == rank
                    {
                        (pot_rtip_state, force_rtip_state) = rti_pot_force(&self.visited_states[i].coord, &s_fragment_coord, self.a[i], self.sigma[i]);
                        pot_rtip_rank += pot_rtip_state;
                        force_rtip_rank += &force_rtip_state;
                    }
                }
            ...
            (Some(atom_add_pot_states), None) =>
            {
                ...
                for i in 0..self.visited_states.len()
                {
                    if i % size == rank
                    {
                        ...
                        (pot_rtip_state, force_rtip_state) = rti_pot_force(&visited_states_coord, &s.coord, self.a[i], self.sigma[i]);
                        pot_rtip_rank += pot_rtip_state;
                        force_rtip_rank += &force_rtip_state;
                    }
                }
            ...
            (Some(atom_add_pot_states), Some(atom_add_pot_s)) =>
            {
                ...
                for i in 0..self.visited_states.len()
                {
                    if i % size == rank
                    {
                        ...
                        (pot_rtip_state, force_rtip_state) = rti_pot_force(&visited_states_coord, &s_fragment_coord, self.a[i], self.sigma[i]);
                        pot_rtip_rank += pot_rtip_state;
                        force_rtip_rank += &force_rtip_state;
                    }
                }

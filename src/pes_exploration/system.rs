...
use crate::math::randomly_rotation::rand_rot;
...
impl System
{
    ...
    /// # Examples
    /// ```
    /// let s: System = System::read_pdb("filename.pdb");
    /// ```
    pub fn read_pdb<P: AsRef<Path> + Debug>(filename: P) -> Self
    {
        let s: ProteinSystem = ProteinSystem::read_pdb(filename);
    ...
    /// ```
    pub fn read_xyz<P: AsRef<Path> + Debug>(filename: P) -> Self
    {
    ...
    /// ```
    pub fn write_xyz<P: AsRef<Path> + Debug>(&self, filename: P, create_new_file: bool, step: usize)
    {

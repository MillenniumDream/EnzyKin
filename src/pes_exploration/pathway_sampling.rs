...
impl RtipPathSampling for RepulsivePot
{
    fn rtip_path_sampling<C: Communicator, P: PES>(&self, comm: &C, real_pes: &P, para: &Para, output_path: &PathBuf)
    {
        ...
        for i in 1..(para.path_samp_para.max_step+1)
        {
            ...
            if rank == ROOT_RANK
            {
                if (i % para.path_samp_para.print_step) == 0
                {
            ...
            dcoord = optimization::min_1d_real_bias(real_pes, &rtip0_pes, &s, &force_total, pot_real+pot_rtip, para.path_samp_para.pot_epsilon*(s.natom as f64));
            s.coord += &dcoord;
            ...
            if pot_real < (pot_real_max - para.path_samp_para.pot_drop)
            {
                add_rtip = false;
            }
            if (add_rtip == false) && (f_real/(s.natom as f64).sqrt() < para.path_samp_para.f_epsilon)
            {
                break
            }
            if pot_real > (pot_real_min + para.path_samp_para.pot_climb) || f_rtip > 1000.0
            {
                break
            }
        }
...
impl IdwmPathSampling for RepulsivePot
{
    fn idwm_path_sampling<C: Communicator, P: PES>(&self, comm: &C, real_pes: &P, para: &Para, output_path: &PathBuf)
    {
        ...
        for i in 1..(para.path_samp_para.max_step+1)
        {
            ...
            match &self.local_min.atom_add_pot
            {
                ...
                None =>
                {
                    ...
                    if add_idwm
                    {
                        ...
                        idwm0_pes.a_min = para.rtip_para.a0 * (i as f64);
                        idwm0_pes.a_ts = idwm0_pes.a_min * para.rtip_para.scale_ts_a0;
            ...
            dcoord = optimization::min_1d_real_bias(real_pes, &idwm0_pes, &s, &force_total, pot_real+pot_idwm, para.path_samp_para.pot_epsilon*(s.natom as f64));
            s.coord += &dcoord;
            ...
            if (add_idwm == false) && (f_real/(s.natom as f64).sqrt() < para.path_samp_para.f_epsilon)
            {
                break
            }
            if pot_real > (pot_real_min + para.path_samp_para.pot_climb) || f_idwm > 1000.0
            {
                break
            }
        }
...
impl RtipPathSampling for AttractivePot
{
    fn rtip_path_sampling<C: Communicator, P: PES>(&self, comm: &C, real_pes: &P, para: &Para, output_path: &PathBuf)
    {
        // Specify the output files
...
impl RtipPathSampling for SynthesisPot
{
    fn rtip_path_sampling<C: Communicator, P: PES>(&self, comm: &C, real_pes: &P, para: &Para, output_path: &PathBuf)
    {
        // Specify the output files
        ...
        for i in 1..(para.path_samp_para.max_step+1)
        {
            ...
            if mol_indices.len() == s.coord.len_of(Axis(0))             // If the molecules for synthesis construct the whole system (i.e. without the other environment atoms or molecules)
            {
                ...
                if add_rtip
                {
                    let rtip0_pes = Rtip0PES
                    {
                        local_min: &final_state,
                        nearby_ts: &_nearby_ts,
                        a_min: -para.rtip_para.a0 * (i as f64),
                        a_ts: 0.0,
            ...
            if pot_real < (pot_real_max - para.path_samp_para.pot_drop) && pot_real > pot_real_initial
            {
                add_rtip = false;
            }
            if (add_rtip == false) && (f_real/(s.natom as f64).sqrt() < para.path_samp_para.f_epsilon)

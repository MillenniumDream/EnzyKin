...
use std::fmt::Debug;
use crate::common::constants::{PI, ANGSTROM_TO_BOHR, BOHR_TO_ANGSTROM, Element};
use crate::common::error::*;
use crate::external::cp2k::*;
use crate::nn::fragment_definition::{FragmentType, Cap, H_ATOM_TYPE, O_ATOM_TYPE};
use crate::pes_exploration::system::System;
use crate::long_ranged::global_para::{AtomPara, AtomParaAu, GlobalPara};
...
pub struct ProteinSystem
{
    ...
    pub long_ranged_para: Vec< Option<AtomParaAu> >,
    pub long_ranged_initialized: bool,
}
...
impl ProteinSystem
{
    ...
    /// ```
    pub fn read_pdb<P: AsRef<Path> + Debug>(filename: P) -> Self
    {
        ...
        return ProteinSystem
        {
            ...
            long_ranged_para: vec![None; natom],
            long_ranged_initialized: false,
        };



        // An inner function for recognition of the fragments read from the PDB file.
        fn recognize_fragment<P: AsRef<Path> + Debug>(fragment_str: &Vec< Vec<&str> >, filename: P) -> Vec<Fragment>
        {
            ...
            match fragment_type
            {
                ...
                FragmentType::Atom(_) | FragmentType::Molecule(_) | FragmentType::Head(_) | FragmentType::Tail(_) =>
                {
                        ...
                        match Element::from_str(fragment_str[natom-1][10])
                        {
                            ...
                            Element::C =>
                            {
                                ...
                                [
                                    Fragment
                                    {
                                        fragment_type: FragmentType::Head(Cap::H),
                                        natom: 1,
                                        coord: coord_head,
                            ...
                            Element::O =>
                            {
                                ...
                                [
                                    ...
                                    Fragment
                                    {
                                        fragment_type: FragmentType::Tail(Cap::O),
                                        natom: 1,
                                        coord: coord_tail,
                        ...
                        match Element::from_str(fragment_str[natom+1][10])
                        {
                            ...
                            Element::H =>
                            {
                                ...
                                [
                                    ...
                                    Fragment
                                    {
                                        fragment_type: FragmentType::Tail(Cap::H),
                                        natom: 1,
                                        coord: coord_tail2,
                                        atom_type: &H_ATOM_TYPE,
                                        pot: 0.0,
    ...
    /// ```
    pub fn write_pdb<P: AsRef<Path> + Debug>(&self, filename: P, create_new_file: bool, step: usize)
    {
        ...
        for i in 0..self.fragment.len()
        {
            match &self.fragment[i].fragment_type
            {
                ...
                FragmentType::Head(cap) =>
                {
                    match cap
                    {
                        // Record the number of Head(H) for the next residue
                        Cap::H => num_head += 1,

                        // Write the cap directly
                        Cap::ACE =>
                        {
                            for j in 0..self.fragment[i].natom
                            {
                                // Warning: The unit of the atomic coordinates should be transformd from Bohr to Angstrom
                                pdb.write_all(format!( "ATOM  {:>5} {:>4} {:>3}  {:>4}    {:8.3}{:8.3}{:8.3}  1.00  0.00          {:>2}\n", index_atom%100000, format!("{:?}", self.fragment[i].atom_type[j]), format!("{:?}", cap), index_residue%10000, self.fragment[i].coord[[j,0]] * BOHR_TO_ANGSTROM, self.fragment[i].coord[[j,1]] * BOHR_TO_ANGSTROM, self.fragment[i].coord[[j,2]] * BOHR_TO_ANGSTROM, format!("{:?}", self.fragment[i].atom_type[j]) ).as_bytes()).expect(&error_file("writing", &filename));
                                index_atom += 1;
                            }
                            index_residue += 1;
                        },
    ...
    /// Input a CP2K force environment, consume the ProteinSystem and set the coordinates of the particles for CP2K
    ///
    /// # Parameters
    /// ```
    ...
    pub fn to_cp2k(&self, force_env: ForceEnv)
    {
        ...
        let pos: &[f64] = self.coord.as_slice().expect(&error_none_value("coord"));
    ...
    /// Transfer the ProteinSystem to an atom-fragment ProteinSystem
    ///
    /// # Examples
    /// ```
    /// ```
    pub fn to_atom_fragment(&self) -> Self
    {
        let mut fragment: Vec<Fragment> = Vec::with_capacity(self.natom);
        for i in 0..self.fragment.len()                // For each fragment in the original ProteinSystem
        {
            for j in 0..self.fragment[i].natom                // For each atom in Fragment i
            {
                // Construct a fragment for Atom j
                let fragment_type: FragmentType = FragmentType::from_str( &format!("{:?}", self.fragment[i].atom_type[j]) );
                let atom_type: &'static [Element] = FragmentType::get_atom_type( &format!("{:?}", self.fragment[i].atom_type[j]) );
                let coord: Array2<f64> = array![[ self.fragment[i].coord[[j, 0]], self.fragment[i].coord[[j, 1]], self.fragment[i].coord[[j, 2]], ],];
                let force: Option< Array2<f64> > = match &self.fragment[i].force
                {
                    Some(force) => Some( array![[ force[[j, 0]], force[[j, 1]], force[[j, 2]], ],] ),
                    None => None,
                };
                let atom_list: Option< Vec<usize> > = match &self.fragment[i].atom_list
                {
                    Some(atom_list) => Some(vec![ atom_list[j] ]),
                    None => None,
                };
        ...
        ProteinSystem
        {
            ...
            fragment,
            long_ranged_dir: self.long_ranged_dir.clone(),
            long_ranged_para: self.long_ranged_para.clone(),
            long_ranged_initialized: self.long_ranged_initialized,
        }
    }
    ...
    /// Get the fixed long-ranged interaction parameters (i.e., the atomic parameters of amino acids
    /// and molecules, which don't change during the calculations) for a protein system
    ///
    /// # Parameters
    /// ```
    /// self: given the mutable reference, reserve the parameters into the long_ranged_para item
    /// ```
    ///
    /// # Examples
    /// ```
    /// s.get_fixed_long_ranged_para(Some("Amber"));
    /// ```
    pub fn get_fixed_long_ranged_para(&mut self)
    {
        // Get the global long-ranged interaction parameters by initialization (from Amber) or loading (from training)
        let global_para: GlobalPara = match &self.long_ranged_dir
        {
            None => GlobalPara::initialize(),
            Some(long_ranged_dir) => GlobalPara::load(long_ranged_dir),
        };

        // Extract the long-ranged interaction parameter for each element iteratively
        let mut long_ranged_para: Vec< Option<AtomParaAu> > = Vec::with_capacity(self.natom);
        let mut n: usize = 0;
        while n < self.fragment.len()               // For each fragment
        {
            match &self.fragment[n].fragment_type
            {
                // For atom, its long-ranged interaction parameters are derived from nn, so a None value is pushed into the Vector
                FragmentType::Atom(_element) =>
                {
                    long_ranged_para.push(None);
                    n += 1;
                },

                // For molecule, extract its parameters from global parameters directly
                FragmentType::Molecule(molecule) =>
                {
                    let fragment_para: &[AtomPara] = global_para.get_fragment_para(&format!("{:?}", molecule));
                    for i in 0..fragment_para.len()
                    {
                        long_ranged_para.push(Some(AtomParaAu::from_atom_para(&fragment_para[i])));
                    }
                    n += 1;
                },
                ...
                {
                    // If it is the last fragment, extract its parameters from global parameters directly
                    let fragment_para: &[AtomPara] = if n == self.fragment.len() - 1
                    {
                        n += 1;
                        global_para.get_fragment_para(&format!("{:?}", amino_acid))
                    }
                    else
                    {
                        // If its next fragment is Tail(Cap::O), then it is a C-terminated amino acid
                        if self.fragment[n+1].fragment_type == FragmentType::Tail(Cap::O)
                        {
                            n += 2;
                            global_para.get_fragment_para(&format!("C{:?}", amino_acid))
                        }
                        // Else, it is an ordinary amino acid, extract its parameters from global parameters directly
                        else
                        {
                            n += 1;
                            global_para.get_fragment_para(&format!("{:?}", amino_acid))
                ...
                FragmentType::Head(cap) =>
                {
                    match &cap
                    {
                        Cap::H =>
                        {
                            // If the current Head is cap H, and it has next fragment, continue
                            let fragment_para: &[AtomPara] = if n < self.fragment.len() - 1
                            {
                                match &self.fragment[n+1].fragment_type
                                {
                                    FragmentType::Head(cap2) =>
                                    {
                                        // If the second Head is cap H, and it has the third fragment, continue
                                        if (*cap2 == Cap::H) && (n < self.fragment.len() - 2)
                                        {
                                            match &self.fragment[n+2].fragment_type
                                            {
                                                // If the third fragment is amino acid, continue
                                                FragmentType::Residue(amino_acid) =>
                                                {
                                                    n += 3;
                                                    global_para.get_fragment_para(&format!("N{:?}", amino_acid))
                                                },
                                                _ => panic!("{}", error_unparameterized_fragment(&format!("{:?}",  &self.fragment[n+2].fragment_type))),
                                            }
                                        }
                                        else
                                        {
                                            panic!("{}", error_unparameterized_fragment(&format!("{:?}", &self.fragment[n+1].fragment_type)));
                                        }
                                    },
                                    _ => panic!("{}", error_unparameterized_fragment(&format!("{:?}", &self.fragment[n+1].fragment_type))),
                                }
                            }
                            else
                            {
                                panic!("{}", error_unparameterized_fragment(&format!("{:?}", &self.fragment[n].fragment_type)));
                            };

                            // First extract parameters of the two Head(H), and then extract parameters of the remaining atoms
                            long_ranged_para.push(Some(AtomParaAu::from_atom_para(&fragment_para[1])));
                            long_ranged_para.push(Some(AtomParaAu::from_atom_para(&fragment_para[2])));
                            long_ranged_para.push(Some(AtomParaAu::from_atom_para(&fragment_para[0])));
                            for i in 3..fragment_para.len()
                            {
                                long_ranged_para.push(Some(AtomParaAu::from_atom_para(&fragment_para[i])));
                            }
                        },
                        ...
                        Cap::ACE =>
                        {
                            let fragment_para: &[AtomPara] = global_para.get_fragment_para(&format!("{:?}", cap));
                            for i in 0..fragment_para.len()
                            {
                                long_ranged_para.push(Some(AtomParaAu::from_atom_para(&fragment_para[i])));
                            }
        ...
        self.long_ranged_para = long_ranged_para;
        self.long_ranged_initialized = true;
    }
}
